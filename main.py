...
async def p2p_buy(message: Message):
    ...
    try:
        ...
        await bot.send_message(chat_id=message.from_user.id, text="""💰 Привет, ты попал на страницу оплаты приватки отзывов и сливов с трансляций.
🔥 Для продолжения покупки нажми на кнопку снизу""", reply_markup=ikb)
